...
to move
  ...
  foreach [self] of patches with [distance myself < perception-range] [ ?1 ->
    ...
    let homeValue (1 / (([distance [home-base] of meTurtle] of ?1 / 50 ) + 0.1)) * (distance [home-base] of meTurtle / 50)
    ...
    foreach [self] of turtles-on ?1 [ ??1 ->
      set conspecificValue conspecificValue + ( 1 / count turtles-on ??1 ) * ( 1 - fighting-ability ) * (1 - tolerance-level meTurtle ??1) * (1 - degree-relatedness meTurtle ??1) * ( 1 - (distance meTurtle / perception-range)) ]
    ...
    ifelse [breed] of meTurtle = females [
      ...
      if count turtles-on ?1 > 0 [ set mateValue (count turtles with [breed = females and patch-here = ?1] / count turtles-on ?1 ) ]
    ]
...
to compete
  let meTurtle self;
  foreach [self] of other turtles-here [ ?1 ->
    ask ?1 [ set energy energy - ([fighting-ability] of myself) * (aggression-cost) * (1 - tolerance-level meTurtle ?1) * (1 - degree-relatedness meTurtle ?1)]
  ]
end
