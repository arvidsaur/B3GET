...
to go
  ask predators [ set victim false ]
...
to check-add-predator
  if random 100 < predation-rate * 100 [
    create-predators 1 [ initialize-predator ]
  ]
end

to move-predators
  set predator-duration predator-duration + 1
  if not hidden? [ set hide-duration hide-duration + 1 ]
  face min-one-of primates [ distance myself ]
  if color = orange [ rt 180 ]
  fd 0.1
  if count primates-here > 0 and color = green [
    ask one-of primates-here [ make-dead ]
    set victim true
    set predator-duration 0
  ]
  if hide-duration > 5 [ set color green ]
  if predator-duration > predation-duration [ die ]
end
...
to check-for-predators
  if alarm-calls? [
    ...
    ask predators with [ color = green ] in-cone 1 20 [ set color orange set hide-duration 0 ]
    ask predators with [ color = orange ] in-radius get-perception-range [ set color orange set hide-duration 0 ]
  ]
end
...
to move
  ...
  if get-life-history != "fetus" and get-life-history != "infant" [
    ...
    ; FOOD
    foreach [self] of other patches with [distance myself <= [get-perception-range] of myself and distance myself > 0.1] [ x ->
      let food-angle atan ([pycor] of x - [ycor] of ME) ([pxcor] of x - [xcor] of ME)
      let food-magnitude ( get-food-weight * [penergy] of x ) / (distance x ^ 2)
    ...
    foreach [self] of primates with [distance myself <= [get-perception-range] of myself and distance myself > 0.1] [ x ->
      let conspecific-angle atan ([ycor] of x - ycor) ([xcor] of x - xcor)
      let tolerance-magnitude get-tolerance-magnitude x
      ...
      [ set conspecific-magnitude ( abs tolerance-magnitude ) * get-winning-likelihood x * get-enemy-weight ]
      set X-magnitude X-magnitude + ((conspecific-magnitude / (distance x ^ 2)) * sin conspecific-angle)
      set Y-magnitude Y-magnitude + ((conspecific-magnitude / (distance x ^ 2)) * cos conspecific-angle)
    ]
...
"Hidden" 1.0 0 -7500403 true "" "plot count predators with [ color = green ]"
"Kills" 1.0 0 -2674135 true "" "plot count predators with [victim]"
