...
to setup-world
  ...
  setup-groups
  setup-baboons
  ;ask groups [ setxy ( mean [xcor] of baboons with [ my-group = myself ] ) ( mean [ycor] of baboons with [ my-group = myself ] ) ]
  reset-ticks
end
...
to go
  ...
  if ceiling ( ticks / 5000 ) = (ticks / 5000) [
    ...
    ;export-world (word "BABOONS20171019 " world-print-name " G" median [generation] of baboons ".csv")
  ]
...
to initialize-baboon
  ...
  set body-size 0
  set conception-rate 0.01
  set lactation-stores 0
  set size 0
  set color blue
  set generation 0
  set my-group nobody
  set mortality-rate 0.01
...
to update-mortality-rate [update]
  if mortality-rate = 0 [ set mortality-rate 0.01 ]
  ifelse update < 0
    [ set mortality-rate ( mortality-rate ^ (1 + abs update) ) ]
    [ set mortality-rate ( mortality-rate ^ (1 / ( 1 + update) )) ]
end

to update-conception-rate [update]
  if conception-rate = 0 [ set conception-rate 0.01 ]
  ifelse update < 0
    [ set conception-rate ( conception-rate ^ (1 + abs update) ) ]
    [ set conception-rate ( conception-rate ^ (1 / ( 1 + update) )) ]
end
...
to update-energy-and-metabolism
  ...
  [ if mother != nobody [ get-lactation-stores-of mother ]]
...
to conception [ value ]
  update-conception-rate ( value / energy-per-conception-unit )
end
...
to-report phenotype-from-genotype [ allele1 allele2 ]
  ...
  if is-string? allele1 and is-string? allele2 [ set phenotype-report first modes (list allele1 allele2) ]
