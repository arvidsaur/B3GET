...
to move
  ...
  if get-life-history != "fetus" and get-life-history != "infant" [
    ...
    ; FOOD
    foreach [self] of other patches with [distance myself <= perception-range  and distance myself > 0.1] [ ?1 ->
   ...
   ;CONSPECIFIC: in-female, in-male, out-female, out-male
    foreach [self] of primates with [distance myself <= perception-range  and distance myself > 0.1] [ ?1 ->
      ...
      [ set conspecific-magnitude 100 * ( abs tolerance-magnitude ) * get-winning-likelihood ?1 * get-enemy-weight ]
