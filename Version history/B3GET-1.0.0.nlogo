...
to setup-groups
  ...
  foreach [self] of groups [ ?1 ->
    let groupPatch one-of patches;
    create-males initial-number-males [ initialize-male nobody nobody ?1 groupPatch ];
    create-females initial-number-females [ initialize-female nobody nobody ?1 groupPatch ];
  ]
end
...
to-report within-group-relatedness
  ...
  foreach [self] of turtles with [breed = males or breed = females] [ ?1 ->
     let meTurtle ?1;
     foreach [self] of other turtles with [(breed = males or breed = females) and my-group = [my-group] of meTurtle] [ ??1 ->
       set relatedness relatedness + degree-relatedness meTurtle ??1;
     ]
  ]
...
to move
  ...
  foreach [self] of patches with [distance myself < perception-range] [ ?1 ->
    ...
    let foodValue ([penergy] of ?1 / patch-max-energy) * (1 - ([energy] of meTurtle / max-energy)) * ( 1 - (distance meTurtle / perception-range));

    ;; HOME RANGE = ( proximity of patch to home ) x ( current distance from home )
    let homeValue 0;
    if [home-base] of meTurtle != 0 [set homeValue (1 / (([distance [home-base] of meTurtle] of ?1 / 50 ) + 0.1)) * (distance [home-base] of meTurtle / 50)]
    ...
    foreach [self] of females-on ?1 [ ??1 ->
      set conspecificValue conspecificValue + ( 1 / count turtles-on ??1 ) * ( 1 - fighting-ability ) * (1 - tolerance-level meTurtle ??1) * (1 - degree-relatedness meTurtle ??1) * ( 1 - (distance meTurtle / perception-range)) * (1 - sex-tolerance meTurtle ??1)  ]
    foreach [self] of males-on ?1 [ ??1 ->
      set conspecificValue conspecificValue + ( 1 / count turtles-on ??1 ) * ( 1 - fighting-ability ) * (1 - tolerance-level meTurtle ??1) * (1 - degree-relatedness meTurtle ??1) * ( 1 - (distance meTurtle / perception-range)) * (1 - sex-tolerance meTurtle ??1)  ]
    ...
    ifelse [breed] of meTurtle = females [
      ...
      if count females-on ?1 > 0 [ set mateValue (count females-on ?1 / count females-on patches with [distance meTurtle < perception-range] )]
    ]

    ;; PREDATORS
    let predatorValue 0;
    if [call-made?] of ?1 [ set predatorValue 1 ]
    if count predators-on ?1 > 0 [ set predatorValue predatorValue + (count predators-on ?1 / count predators-on patches with [distance meTurtle < perception-range] )]
    if predatorValue > 0 and play-alarm-calls? [ sound:play-note "Bird Tweet" 40 - random 5 40 - random 5  0.3
                                                 ask ?1 [ set call-made? true ]]
...
to compete
  let meTurtle self;
  foreach [self] of other males-here [ ?1 ->
    ask ?1 [ set energy energy - ([fighting-ability] of myself) * (aggression-cost) * (1 - tolerance-level meTurtle ?1) * (1 - degree-relatedness meTurtle ?1) * (1 - sex-tolerance meTurtle ?1)]
  ]
  foreach [self] of other females-here [ ?1 ->
    ask ?1 [ set energy energy - ([fighting-ability] of myself) * (aggression-cost) * (1 - tolerance-level meTurtle ?1) * (1 - degree-relatedness meTurtle ?1) * (1 - sex-tolerance meTurtle ?1)]
  ]
  foreach [self] of other predators-here [ ?1 ->
    ask ?1 [ set predator-age predator-age + ([fighting-ability] of myself) * (aggression-cost)]
  ]
end
...
to update-group-food-graph
  ...
  foreach [self] of groups [ ?1 ->
    let foodValue sum [food-eaten] of turtles with [my-group = ?1] / count turtles with [my-group = ?1]
    plotxy [group-size] of ?1 foodValue
    plot-pen-down
    plot-pen-up
  ]
end
...
to update-group-predation-graph
  ...
  foreach [self] of groups [ ?1 ->
    if [predation-count] of ?1 > 0 [
      plotxy [group-size] of ?1 [predation-count] of ?1
      plot-pen-down
      plot-pen-up
    ]
  ]
...
to update-competition-food-graph
  ...
  foreach [self] of females [ ?1 ->
    plotxy [fighting-ability] of ?1 [food-eaten] of ?1
    plot-pen-down
    plot-pen-up
  ]
end
...
to update-competition-mates-graph
  ...
  foreach [self] of males [ ?1 ->
    plotxy [fighting-ability] of ?1 [mate-number] of ?1
    plot-pen-down
    plot-pen-up
  ]
