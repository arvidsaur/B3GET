...
to setup-parameters
  ...
  set action-list [ "attack" "mate" "groom" "maintain" "grow" "conception" "lactate" "toward" "away" "join" "leave" "forage" ]
  set self-action-list [ "maintain" "grow" "conception" "lactate" "leave" "forage" ]
...
to go
  ...
  ; ask baboons [ print (word current-energy " " energy )]
  ask groups [ if not any? baboons with [ my-group = myself ] [ die ]]
  ask baboons [ update-energy-and-metabolism ]
  ask baboons [ make-decisions ]
  ask baboons [ update-life-history update-fertility update-status ]
  ;if ceiling ( ticks / 10000 ) = (ticks / 10000) [ export-world (word "BABOONS20170926 G" median [generation] of baboons ".csv")]
  tick
end
...
to update-energy-and-metabolism
  set energy current-energy ;energy-received-per-timestep
...
to make-decisions
  ...
  let X-magnitude 0.0001 * one-of [ -1 1 ]
  let Y-magnitude 0.0001 * one-of [ -1 1 ]
...
to forage [ value ]
  set current-energy value * energy-to-forage-gains
end
...
@#$#@#$#@
Code taken from model 3_20161111
@#$#@#$#@
default
true
0
